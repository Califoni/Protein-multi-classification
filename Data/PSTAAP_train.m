...
%1
[a1 b1]=fastaread('(1)Train9279.txt');
[a2 b2]=fastaread('(2)Train710.txt');
[a3 b3]=fastaread('(3)Train600.txt');
[a4 b4]=fastaread('(4)Train454.txt');
[a5,b5]=fastaread('(5)Train561.txt');
[a6 b6]=fastaread('(6)Train251.txt');
[a7 b7]=fastaread('(7)Train360.txt');
[a8 b8]=fastaread('(8)Train88.txt');
[a9 b9]=fastaread('(9)Train153.txt');
[a10 b10]=fastaread('(10)Train454.txt');
[a1_1 b1_1]=fastaread('(11)Train73.txt');
...
M1=length(b1{1,1});
M11=length(b11{1,1});
F1=zeros(20^3,M1-2);%记录每个氨基酸在每个位置出现的频率
...
M2=length(b2{1,1});
M21=length(b21{1,1});
F2=zeros(20^3,M2-2);%记录每个氨基酸在每个位置出现的频率
...
M3=length(b3{1,1});
M31=length(b31{1,1});
F3=zeros(20^3,M3-2);%记录每个氨基酸在每个位置出现的频率
...
M4=length(b4{1,1});
M41=length(b41{1,1});
F4=zeros(20^3,M4-2);%记录每个氨基酸在每个位置出现的频率
...
M5=length(b5{1,1});
M51=length(b51{1,1});
F5=zeros(20^3,M5-2);%记录每个氨基酸在每个位置出现的频率
...
M6=length(b6{1,1});
M61=length(b61{1,1});
F6=zeros(20^3,M6-2);%记录每个氨基酸在每个位置出现的频率
...
M7=length(b7{1,1});
M71=length(b71{1,1});
F7=zeros(20^3,M7-2);%记录每个氨基酸在每个位置出现的频率
...
M8=length(b8{1,1});
M81=length(b81{1,1});
F8=zeros(20^3,M8-2);%记录每个氨基酸在每个位置出现的频率
...
M9=length(b9{1,1});
M91=length(b91{1,1});
F9=zeros(20^3,M9-2);%记录每个氨基酸在每个位置出现的频率
...
M10=length(b10{1,1});
M101=length(b101{1,1});
F10=zeros(20^3,M10-2);%记录每个氨基酸在每个位置出现的频率
...
M1_1=length(b1_1{1,1});
M111=length(b111{1,1});
F1_1=zeros(20^3,M1_1-2);%记录每个氨基酸在每个位置出现的频率
...
save("PSTAAP.mat","PSTAAP1","PSTAAP2","PSTAAP3","PSTAAP4","PSTAAP5","PSTAAP6","PSTAAP7","PSTAAP8","PSTAAP9","PSTAAP10","PSTAAP11")